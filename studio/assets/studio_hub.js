...
(function (global) {
  ...
  /* ------------------------------------------------------------------ *
   * I TIPI DI MATERIALE, in un posto solo                               *
   *                                                                     *
   * La fetta 2 aveva lasciato l'hub «agnostico rispetto al materiale» a  *
   * parole, ma nel codice il tipo era un `this.tipo === 'quiz' ? A : B`  *
   * ripetuto in quattordici punti. Aggiungere il terzo materiale voleva  *
   * dire toccarli tutti e quattordici, e il quindicesimo dimenticato non *
   * si sarebbe visto: sarebbe uscito un hub che dice «carte» sopra una   *
   * lista di esercizi. Adesso il tipo e' UNA riga di tabella, e i punti  *
   * che la leggono non sanno piu' quale materiale stanno servendo.       *
   *                                                                     *
   * campo/motivo  = come si chiamano nel catalogo il conteggio e il      *
   *                 motivo di una parte spenta (li scrive catalogo_hub)  *
   * unita         = singolare/plurale per i conteggi a schermo           *
   * filtri        = quali comandi hanno senso: le guide non hanno ne'    *
   *                 difficolta' ne' argomenti, e mostrarne i comandi     *
   *                 sarebbe promettere un filtro che non filtra          *
   * aggregato     = il file di materia da cui si pesca (nessuno tranne i *
   *                 quiz scarica per lezione: vedi «perche' non 60 file»)*
   * ------------------------------------------------------------------ */
  var TIPI = {
    flashcards: {
      campo: 'carte', motivo: 'motivoCarte',
      unita: ['carta', 'carte'], cosa: 'con le carte',
      avvia: 'Comincia il ripasso', attesa: 'Apro le carte…',
      filtri: { diff: true, tag: true },
      modi: [{ v: 'tutto', t: 'Tutte le carte' },
             { v: 'bestie', t: 'Le mie bestie nere' }],
      cartella: 'flashcards', prefisso: '_mazzo-', elenco: 'carte'
    },
    quiz: {
      campo: 'quiz', motivo: 'motivoQuiz',
      unita: ['domanda', 'domande'], cosa: 'con le domande',
      avvia: 'Comincia la serie', attesa: 'Scelgo le lezioni…',
      filtri: { diff: true, tag: true },
      modi: [{ v: 'tutto', t: 'Tutte le domande' },
             { v: 'sbagliate', t: 'Solo quelle che ho sbagliato' }],
      /* i quiz sono gli UNICI a scaricare per lezione: le domande intere
         pesano 8,6 MB e aggregarle sarebbe assurdo (vedi §perche'). */
      perLezione: true, cartella: 'quiz', prefisso: '_facce-', elenco: 'domande'
    },
    esercizi: {
      campo: 'esercizi', motivo: 'motivoEsercizi',
      unita: ['esercizio', 'esercizi'], cosa: 'con gli esercizi',
      avvia: 'Comincia gli esercizi', attesa: 'Apro gli esercizi…',
      filtri: { diff: true, tag: true },
      /* il modo qui NON costringe a scaricare: quanti svolti e quanti di
         pratica ha ogni parte sta gia' nel catalogo in pagina */
      modi: [{ v: 'tutto', t: 'Tutti gli esercizi' },
             { v: 'svolto', t: 'Solo gli svolti (col metodo passo passo)' },
             { v: 'pratica', t: 'Solo quelli di pratica' }],
      cartella: 'esercizi', prefisso: '_serie-', elenco: 'esercizi'
    },
    procedure: {
      campo: 'guide', motivo: 'motivoGuide',
      unita: ['guida', 'guide'], cosa: 'con le guide',
      avvia: 'Apri le guide', attesa: 'Apro le guide…',
      /* niente difficolta' e niente argomento: le guide non li hanno nel
         sorgente, e un filtro che non filtra e' peggio di un filtro assente */
      filtri: { diff: false, tag: false },
      modi: null,
      cartella: 'procedure', prefisso: '_guide-', elenco: 'guide'
    }
  };

  function tipoValido(t) {
    return TIPI.hasOwnProperty(t) ? t : 'flashcards';
  }
  ...
  function Hub(radice, opzioni) {
    ...
    this.doc = doc;
    this.radice = radice;
    this.tipo = tipoValido(String(opzioni.tipo || radice.getAttribute('data-hub') || 'flashcards'));
    this.T = TIPI[this.tipo];
    ...
    this.mazzi = {};          /* materia -> aggregato di materia gia' aperto */
    this.filtri = {
      materia: '', ud: '', ep: '', parte: '',
      diff: {}, tag: '', modo: 'tutto'
    };
  ...
  Hub.prototype.applicaUrl = function () {
    ...
    for (i = 0; i < pezzi.length; i++) {
      ...
      else if (k === 'modo' && this.modoValido(v)) this.filtri.modo = v;
  ...
  /* Un `modo` arriva dall'indirizzo, cioe' da fuori: si accetta solo se e'
     uno di quelli che QUESTO materiale ha davvero. Prima l'elenco era scritto
     a mano nella lettura dell'URL, e sarebbe rimasto indietro al primo modo
     nuovo — accettando un valore che nessun menù puo' produrre. */
  Hub.prototype.modoValido = function (v) {
    var m = this.T.modi || [], i;
    for (i = 0; i < m.length; i++) if (m[i].v === v && v !== 'tutto') return true;
    return false;
  };
  ...
  Hub.prototype.lezioni = function () {
    ...
    for (i = 0; i < eps.length; i++) {
      ...
      for (j = 0; j < (e.parti || []).length; j++) {
        var p = e.parti[j];
        var quanti = p[this.T.campo] || 0;
        var motivo = p[this.T.motivo];
        fuori.push({
          k: e.slug + '-' + p.parte,
          ...
          /* i sotto-conteggi che il catalogo gia' porta: servono a rispondere
             ESATTAMENTE al filtro «solo gli svolti» senza scaricare niente */
          svolti: p.svolti || 0, pratica: p.pratica || 0
        });
      }
    }
    this._lezioni = fuori;
  ...
  Hub.prototype.costruisci = function () {
    ...
    /* difficoltà: chip a interruttore, come nei motori — stessa lingua
       visiva, stesso gesto, e `aria-pressed` porta lo stato senza colore.
       Esce solo se il materiale ce l'ha (le guide no). */
    n.chips = [];
    if (this.T.filtri.diff) {
    ...
    if (this.T.modi) n.modo = this.campoSelect(form, 'Cosa studiare', 'modo');
  ...
  Hub.prototype.aggiornaSelettori = function () {
    ...
    if (n.modo) this.opzioniIn(n.modo, this.T.modi, f.modo);
  };
  ...
  Hub.prototype.azzeraFiltri = function () {
    ...
    if (this.nodi.tag) this.nodi.tag.value = '';
    this.aggiornaSelettori();
    this.cambiato();
  };

  Hub.prototype.cambiato = function () {
    this.filtri.modo = (this.nodi.modo && this.nodi.modo.value) || 'tutto';
    this.aggiornaConteggio();
    this.disegnaCatalogo();
  };
  ...
  Hub.prototype.aggiornaConteggio = function () {
    ...
    var quanti = 0;
    var esatto = true;
    var unita = this.T.unita;
    if (this.T.perLezione) {
      ...
      var conto = this.elementiAmmessi(ammK);
      quanti = conto.quanti;
      lez = conto.lezioni;
      if (!quanti) {
        this.testo(n.conteggio, 'Nessun elemento con questi filtri: l\'argomento ' +
          'e la difficoltà che hai scelto non stanno insieme in nessuna ' +
          unita[0] + ' della selezione.');
        n.avvia.disabled = true;
        return;
      }
    } else {
      /* Il conto dal solo CATALOGO. Per gli esercizi «solo gli svolti» ci
         rientra: quanti svolti e quanti di pratica ha ogni parte è già in
         pagina, quindi il numero è esatto senza scaricare un byte. */
      lez = 0;
      for (i = 0; i < amm.length; i++) {
        var c = this.quantiDaCatalogo(amm[i]);
        if (c > 0) { quanti += c; lez++; }
      }
      if (this.servonoFacce()) esatto = false;
      if (!lez) {
        this.testo(n.conteggio, 'Nessun materiale di questo tipo nelle lezioni ' +
          'scelte: cambia «cosa studiare» o allarga la selezione.');
        n.avvia.disabled = true;
        return;
      }
    }
  ...
  /* Quanti elementi di QUESTA parte passano i filtri che il catalogo sa già
     rispondere (cioè tutto tranne difficoltà e argomento). */
  Hub.prototype.quantiDaCatalogo = function (l) {
    if (this.tipo === 'esercizi') {
      if (this.filtri.modo === 'svolto') return l.svolti;
      if (this.filtri.modo === 'pratica') return l.pratica;
    }
    return l.n;
  };
  ...
  Hub.prototype.preparaTag = function () {
    ...
    /* materiale senza argomenti: il campo non esiste nemmeno, e chiamare qui
       vorrebbe dire scrivere dentro un nodo che non c'e' */
    if (!this.T.filtri.tag || !n.tagNota) return;
    if (this._tagInCorso) return;
    this._tagInCorso = true;
    this.testo(n.tagNota, 'cerco gli argomenti…');
    var attesa = this.T.perLezione
  ...
  Hub.prototype.tagDaMazzi = function (mazzi) {
    ...
    for (i = 0; i < mazzi.length; i++) {
      var voci = this.elementiIn(mazzi[i]);
      for (j = 0; j < voci.length; j++) {
        if (!amm[voci[j].k]) continue;
        var tg = voci[j].tag || [];
  ...
  /* L'array dentro l'aggregato di materia: `carte`, `esercizi`, `guide`.
     Il nome sta nella tabella dei tipi, non sparso nel codice. */
  Hub.prototype.elementiIn = function (aggregato) {
    var v = aggregato && aggregato[this.T.elenco];
  ...
  Hub.prototype.avviso = function (err) {
    ...
    n.avvia.disabled = false;
    this.testo(n.avvia, this.T.avvia);
  };
  ...
  /* PORTARE LA PAGINA DOVE SUCCEDE LA COSA. Il motore si monta SOTTO i filtri e
     la barra dei bottoni: su un telefono sono ~550px di filtri piu' la barra,
     quindi chi tocca «Comincia la serie» non vede muoversi niente — l'unico
     segnale e' il bottone che si spegne, e chi non indovina di dover scorrere
     tocca di nuovo o se ne va. Misurato il 2026-09-19: in tutti i motori di
     /studio non c'era un solo scrollIntoView. Il pattern giusto era gia' in
     casa, in `vuoto()`: tabindex -1 + focus, che porta anche chi naviga da
     tastiera e chi usa un lettore di schermo. */
  Hub.prototype.portaAlMotore = function () {
    var n = this.nodi;
    if (!n || !n.motore) return;
    try { n.motore.scrollIntoView({ block: 'start', behavior: 'smooth' }); } catch (e) {
      try { n.motore.scrollIntoView(true); } catch (e2) { }
    }
  };
  ...
  Hub.prototype.attesa = function (testo) {
    var doc = this.doc, n = this.nodi;
    svuota(n.motore);
    var p = el(doc, 'div', { 'class': 'hub-attesa', role: 'status', tabindex: '-1' }, testo);
    n.motore.appendChild(p);
    this.portaAlMotore();
    try { p.focus(); } catch (e) { }
    return p;
  ...
  Hub.prototype.comincia = function () {
    ...
    if (this.T.perLezione) this.cominciaQuiz();
    else this.cominciaAggregato();
  };
  ...
  /* ---- materiali che scendono AGGREGATI per materia -------------------- *
   * flashcards, esercizi, guide: un file per materia, uno scaricamento e   *
   * una decifratura al posto di sessanta. Il codice è UNO solo: cambia il  *
   * nome del file, il nome dell'array dentro e il motore a cui si          *
   * consegna — tutte e tre cose che stanno nella tabella dei tipi.         *
   * ---------------------------------------------------------------------- */
  ...
  Hub.prototype.caricaMazzi = function () {
    ...
    for (i = 0; i < m.length; i++) {
      (function (mat) {
        attese.push(scarica(self.base + self.T.cartella + '/' + self.T.prefisso +
          mat + '.json').then(function (d) { self.mazzi[mat] = d; return d; }));
      })(m[i]);
    }
    return global.Promise.all(attese);
  };

  /* Gli aggregati delle materie attive sono GIÀ in memoria? Se sì il conteggio
     per argomento e difficoltà si fa esatto senza scaricare niente. */
  ...
  /* Passa i filtri di dettaglio (difficoltà, argomento, «cosa studiare»)?
     Quelli strutturali li ha già applicati `ammesse()` sulle lezioni. */
  Hub.prototype.passaFiltri = function (x) {
    if (this.T.filtri.diff && this.quanteDiff() && !this.filtri.diff[x.diff]) return false;
    if (this.T.filtri.tag && this.filtri.tag &&
        indiceIn(x.tag || [], this.filtri.tag) < 0) return false;
    if (this.tipo === 'esercizi' &&
        (this.filtri.modo === 'svolto' || this.filtri.modo === 'pratica') &&
        x.tipo !== this.filtri.modo) return false;
    return true;
  };

  /* Quanti elementi passano TUTTI i filtri, con gli aggregati in memoria. */
  Hub.prototype.elementiAmmessi = function (ammK) {
    var m = this.materieAttive(), tot = 0, lez = {}, i, j;
    for (i = 0; i < m.length; i++) {
      var lista = this.elementiIn(this.mazzi[m[i]]);
      for (j = 0; j < lista.length; j++) {
        var c = lista[j];
        if (!ammK[c.k] || !this.passaFiltri(c)) continue;
        tot++;
        lez[c.k] = 1;
      }
    }
    ...
    return { quanti: tot, lezioni: n };
  };

  Hub.prototype.cominciaAggregato = function () {
    var self = this;
    var amm = this.ammesse(), ammK = {}, i;
    for (i = 0; i < amm.length; i++) ammK[amm[i].k] = amm[i];
    this.attesa(this.T.attesa);
    var t0 = (new Date()).getTime();
    this.caricaMazzi().then(function (mazzi) {
      var voci = [], j, z;
      for (j = 0; j < mazzi.length; j++) {
        var lista = self.elementiIn(mazzi[j]);
        for (z = 0; z < lista.length; z++) {
          var c = lista[z];
          if (!ammK[c.k] || !self.passaFiltri(c)) continue;
          voci.push(c);
        }
      }
      self.fine();
      /* gli aggregati ora sono in memoria: il conteggio in cima puo' smettere
         di dire «fino a» e diventare esatto */
      self.aggiornaConteggio();
      if (!voci.length) {
        self.vuoto('Nessun materiale con questi filtri',
          'Le lezioni scelte ci sono, ma nessun elemento ha insieme tutto quello ' +
          'che hai chiesto. Togli un filtro e riprova.');
        return;
      }
      var materie = {};
      for (j = 0; j < voci.length; j++) materie[ammK[voci[j].k].materia] = 1;
      var quante = 0, unica = '';
      for (var mm in materie) if (materie.hasOwnProperty(mm)) { quante++; unica = mm; }

      /* LA TAVOLA DEI RIMANDI segue le voci. Senza questa unione una serie
         composta dall'hub perderebbe TUTTI i rimandi — non con un errore, ma
         con un aiuto che semplicemente non compare: il buco più silenzioso.
         Le chiavi negli aggregati sono già `<lezione>#<id>`, quindi unire le
         tavole di più materie non può sovrapporre niente. */
      var rimandi = {}, kk;
      for (j = 0; j < mazzi.length; j++) {
        var tav = mazzi[j] && mazzi[j].rimandi;
        if (!tav || typeof tav !== 'object') continue;
        for (kk in tav) if (tav.hasOwnProperty(kk)) rimandi[kk] = tav[kk];
      }

      svuota(self.nodi.motore);
      var punto = el(self.doc, 'div', {});
      self.nodi.motore.appendChild(punto);
      self.motoreVivo = self.montaMotore(punto, voci, quante === 1 ? unica : '', rimandi);
      self.misura(t0, voci.length, self.T.unita[1]);
    })['catch'](function (err) {
      self.fine();
      svuota(self.nodi.motore);
      self.avviso(err);
  ...
  /* L'unico punto in cui l'hub sa quale motore sta servendo. Se un motore
     manca (asset non copiato) NON si finge: si dice, col nome del file. */
  Hub.prototype.montaMotore = function (punto, voci, materia, rimandi) {
    var titolo = this.titoloSelezione();
    if (this.tipo === 'flashcards') {
      if (!global.Flashcards) return this.motoreAssente(punto, 'flashcards.js');
      return global.Flashcards.monta(punto, {
        ...
        materia: materia, lezione: titolo, carte: voci
      }, {
        chiave: CHIAVE_CARTE, titolo: titolo,
        avvio: this.filtri.modo === 'bestie' ? 'bestie' : ''
      });
    }
    if (this.tipo === 'esercizi') {
      if (!global.Esercizi) return this.motoreAssente(punto, 'esercizi.js');
      return global.Esercizi.monta(punto, {
        materia: materia, lezione: titolo, esercizi: voci,
        rimandi: rimandi || {}
      }, { chiave: CHIAVE_CARTE, titolo: titolo, materia: materia });
    }
    if (!global.Procedure) return this.motoreAssente(punto, 'procedure.js');
    return global.Procedure.monta(punto, {
      materia: materia, lezione: titolo, guide: voci
    }, { titolo: titolo, materia: materia });
  };

  Hub.prototype.motoreAssente = function (punto, file) {
    var doc = this.doc;
    var p = el(doc, 'div', { 'class': 'hub-avviso', role: 'alert' });
    p.appendChild(el(doc, 'h3', {}, 'Il motore non è stato caricato'));
    p.appendChild(el(doc, 'p', {},
      'Il materiale è arrivato, ma manca lo script che lo fa funzionare (' +
      file + '). Ricarica la pagina; se il problema resta, è un difetto del ' +
      'sito e va segnalato.'));
    punto.appendChild(p);
    return null;
  };
