...
(function (global) {
  ...
  App.prototype.giudica = function (esito) {
    ...
    /* IL FATTO ESCE DAL DISPOSITIVO (2026-09-19). Lo stato di studio vive in
       localStorage, quindi sta su UN browser: chi studia dal telefono e poi dal
       computer ricominciava da capo, e «quali argomenti sbaglio di piu'» non si
       poteva rispondere. `studioEvento` lo mette in coda (lo definisce il sito
       nella copia privata, vedi site_build) e lo spedisce piu' tardi. Se non
       c'e' — sito gratuito, o nessun account — qui non succede niente. */
    if (global.studioEvento) {
      var sNuovo = this.stato.carte[carta.id];
      var gg = function (ms) {
        try { return new Date(ms).toISOString().slice(0, 10); } catch (e) { return null; }
      };
      try {
        global.studioEvento({
          kind: 'flashcard',
          item: carta.id,
          lezione: carta.k || null,
          materia: this.mazzo ? this.mazzo.materia : null,
          tag: carta.tags,
          diff: carta.diffGrezza || carta.diff,
          esito: esito === 'ok' ? 'ok' : 'ko',
          ms: this.t0 ? adesso - this.t0 : null,
          track: 'carte',
          box: nuovaB,
          scade: gg(sNuovo && sNuovo.d),
          ultimo: gg(adesso)
        });
      } catch (e) { /* una statistica non deve mai fermare lo studio */ }
    }
