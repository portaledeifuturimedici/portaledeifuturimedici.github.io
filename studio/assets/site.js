...
(function () {
  ...
  if (grid) {
    ...
    /* la via d'uscita dal vicolo cieco: chi ha cercato una parola che non c'e'
       aveva davanti solo «Nessun episodio», e doveva svuotare il campo a mano */
...
/* --- eventi di studio: coda locale, invio a blocchi (2026-09-19) --- */
(function () {
  'use strict';
  ...
  function leggi() {
    try { return JSON.parse(localStorage.getItem(CODA) || '[]'); } catch (e) { return []; }
  }
  function salva(v) {
    try { localStorage.setItem(CODA, JSON.stringify(v.slice(-MAX_CODA))); } catch (e) {}
  }
  ...
  window.studioEvento = function (ev) {
    ...
    c.push(ev);
    salva(c);
    if (c.length >= BLOCCO) manda();
  };

  function manda() {
    if (inviando || !tok()) return;
    var c = leggi();
    if (!c.length) return;
    var lotto = c.slice(0, BLOCCO);
    inviando = true;
    fetch(API + '/studio/eventi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + tok() },
      body: JSON.stringify({ eventi: lotto })
    }).then(function (r) {
      inviando = false;
      ...
      var resto = leggi().slice(lotto.length);
      salva(resto);
      if (resto.length) manda();
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', manda);
  } else { manda(); }
  setInterval(manda, 120000);
...
(function () {
  try {
    var n = localStorage.getItem('appunti-chi');
    if (!n || !localStorage.getItem('appunti-tok')) return;
    document.querySelectorAll('a.nav-acc[data-nome]').forEach(function (a) {
      a.textContent = n;
      a.title = 'Sei entrato come ' + n + ': apri il tuo profilo';
    });
  } catch (e) {}
})();
